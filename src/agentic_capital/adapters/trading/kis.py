...
class KISTradingAdapter(TradingPort):
    ...
    async def _submit_domestic_order(self, order: Order) -> OrderResult:
        ...
        try:
            ...
            if data.get("rt_cd") != "0":
                ...
                if (
                    self._session.is_paper
                    and broker_order.price
                    and self._can_fill_rejected_paper_domestic_order(broker_order, data)
                ):
    ...
    def _can_fill_rejected_paper_domestic_order(self, order: Order, data: dict[str, Any]) -> bool:
        if order.side == OrderSide.BUY:
            return True
        return (
            self._domestic_paper_cash_reject(data)
            or str(order.exchange or "").upper() == "NXT"
            or self._has_local_paper_domestic_close_position(order)
        )
