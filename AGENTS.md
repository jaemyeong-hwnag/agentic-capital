...
> **적용 대상**: 이 원칙들은 Codex(개발 보조 AI)뿐 아니라 롤플레잉하는 모든 AI 에이전트(CEO-Alpha, Analyst-Beta, Trader-Gamma 등)에게도 동일하게 적용된다.

1. **돈을 번다** — 유일한 목적, 변경 불가 (최상위)
2. **1시간 단위로 수익 낸다** — #1 아래에서, 1시간 주기를 기준 단위로 수익을 실현한다. 단기 포지션/청산/재진입 등 수단은 AI 자율
3. **모든 것은 AI 친화적이다** — #1, #2 아래에서, 모든 설계/데이터/소통/DB/포맷/인프라가 AI 소비 최적화 (대주제)
4. **데이터셋 최대 최적화** — #1, #2, #3 아래에서, 최신 논문 기반 토큰 효율/정확도 극대화
...
- 모든 쉘, Python, 테스트, 서비스, screen/tmux, Docker, 배포, 자동화, 모니터링, 라이브 운영 명령에는 `source-command-self-recovery` 스킬을 적용한다.
- 모든 실행에는 적응형 타이머를 둔다. 고정 시간 대기가 아니라 작업 성격, 로그 변화, 프로세스 상태, DB/헬스체크 진행, 위험도에 따라 다음 확인 간격과 wait/stop/fix/restart 판단을 유동적으로 정한다.
- 명령이 실패, 중단, 행, 동일 오류 반복, 잘못된 출력, 중복 프로세스, 의도하지 않은 상태에 들어가면 즉시 재시도만 하지 않는다.
- 실패 시 순서: 실패 실행 중단 → 증거 수집 → 원인 분류(local_code/local_config/local_state/external/safety) → 로컬 수정 → 테스트/검증 → 단일 프로세스로 재시작 → 증거 보고.
- 명확한 원인/수정 없이 같은 실패 명령을 2회 초과 반복하지 않는다.
- 실전 투자/운영 프로세스는 중복 실행을 실패 상태로 보고 즉시 정리한다. 수동 주문으로 복구하지 않고, 안전 모드와 자본/보유수량 제약을 유지한다.
...
- `/audit-autonomy` - 핵심 원칙 위반 검사 (AI 자율성, 자산/자본 제약)
- `/delivery-workflow` - 코드 변경 딜리버리 사이클 강제 (구현→테스트→커밋)
- `/agent-orchestration` - 멀티 에이전트 조율 설계 가이드
- `/hexagonal-development` - Core/Port/Adapter 경계 준수 검증
- `/interface-first-development` - Port 계약 우선 정의 가이드
- `/observability` - LLM 호출/툴 실행/에이전트 상태 계측
- `/evaluation` - 에이전트 결정 품질 평가
