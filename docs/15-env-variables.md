...
| `LOCAL_FINANCE_DEFAULT_SYMBOLS` | 선택 | Trader finance cycle이 순환 평가할 기본 종목 유니버스. `005930,us_stock:AAPL`, `AAPL@us_stock`, `kr_options:K200_CALL_ATM` 형식을 지원하며 기본값은 `us_stock:NVDA,us_stock:TQQQ,us_stock:SOXL,us_stock:TSLA,005930,069500,233740,252670,251340,114800,us_stock:AAPL`. 열린 시장 후보는 quote/OHLCV 기반 read-only BUY signal이 있으면 cycle 순번보다 우선한다. KRX/NXT 세션은 `KRX`, `KRX_*`, `NXT`, `NXT_*` 값을 한국 주식 열린 경로로 처리한다. `kr_options`는 콜옵션만 허용한다 |
...
| `LOCAL_FINANCE_PAPER_ORDER_EXECUTION_ENABLED` | 선택 | 기본값 `true`. Trader finance sidecar가 검증된 BUY/SELL paper intent를 반환하고 KIS paper/live-futures 안전 조건을 통과하면 agentic-capital이 paper 주문을 제출한다. KRX 현물은 KIS paper API를 먼저 사용하되 가격 있는 paper BUY가 거절되면 `PAPER-KR-*` 로컬 paper fill로 기록하고, 해외 현물은 로컬 paper fill로 기록한다. `kr_options`는 콜옵션만 허용하고 paper mode에서는 KOSPI200 기반 로컬 프리미엄으로 `PAPER-CALL-*` 체결을 기록할 수 있으며 live 주문 권한이 아니다 |
| `LOCAL_FINANCE_PAPER_PROBE_ON_MODEL_LOOP` | 선택 | 기본값 `true`. 모든 필수 tool evidence가 있는데 finance decision model이 `CALL_TOOL`만 반복하거나 `WAIT`/`HOLD`/`OBSERVE` no-order만 반환하면, open market/cash/risk/quote 조건과 `market_signal=BUY` 성과 후보 조건에서 작은 paper scout BUY를 자동 제출한다. decision stage payload에는 같은 조건을 `paper_trade_candidate`로 먼저 전달해 finance model이 직접 `BUY`/`SELL` + `would_submit_order=true`를 낼 수 있게 한다. 한국 주식 paper scout의 open market에는 `KRX*`/`NXT*` 세션이 포함된다. KIS paper API가 가격 있는 한국 주식 paper BUY, `NXT` after-session scout 주문, 또는 로컬 paper 포지션 SELL close를 거절하면 paper safety 안에서 `PAPER-KR-*` 로컬 체결로 기록한다. 명시적 콜옵션은 `PAPER-CALL` paper bridge로 1계약만 허용한다 |
