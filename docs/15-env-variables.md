...
| `LOCAL_FINANCE_PAPER_PROBE_ON_MODEL_LOOP` | 선택 | 기본값 `true`. 모든 필수 tool evidence가 있는데 finance decision model이 `CALL_TOOL`만 반복하거나 `WAIT`/`HOLD`/`OBSERVE` no-order만 반환하면, open market/cash/risk/quote 조건과 `market_signal=BUY` 성과 후보 조건에서 작은 paper scout BUY를 자동 제출한다. 명시적 콜옵션은 `PAPER-CALL` paper bridge로 1계약만 허용한다 |
