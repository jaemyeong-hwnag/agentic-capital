...
줄인다. psychology sidecar가 partial JSON, 비정형 JSON, 또는 order/capital/risk-limit 경계 위반 필드를 반환하면 Agentic Capital은
`schema_repaired_context_only` soft context로 축소해 기록한다. 이 경우도 원본 `agent_state_patch`나
`BUY`/`SELL`, quantity, order permission, capital allocation, risk limit override를 finance/order 권한으로 승격하지 않는다.
수리된 payload에는 원문 preview를 싣지 않고 `raw_output_sha256`/`raw_output_chars` fingerprint만 남겨,
원문 속 order/clinical 단어가 다시 validator를 통과하며 soft context를 깨는 일을 막는다.
