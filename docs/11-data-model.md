...
> **모든 것을 논문 참고 가능 수준으로 기록한다.**
> 페르소나 상태, 성격 변화, 감정 변동, 투자 결정, 결과, 인사 이벤트, 권한 변경, 에이전트 간 대화, 조직 변경 — **전부 DB에 저장**.

### 논문급 기록 요건

| 요건 | 설명 |
|------|------|
| **재현성 (Reproducibility)** | 동일 초기 조건으로 시뮬레이션 재실행 가능 |
| **추적성 (Traceability)** | 모든 결정의 이유, 맥락, 결과를 역추적 가능 |
| **스냅샷 (Snapshot)** | 매 판단 시점의 성격/감정/포트폴리오/시장 상태 동시 기록 |
| **시계열 (Time-Series)** | 모든 변화를 타임스탬프와 함께 시계열로 저장 |
| **내보내기 (Export)** | 전체 데이터를 Parquet/CSV로 추출하여 분석 가능 |
| **메타데이터** | 시뮬레이션 버전, 모델 버전, 파라미터 설정 등 실험 메타 기록 |
